(function() {
  $(function() {
    ...
    // Get initial value
  $selectValue.text($('#province').val());
	$selectValue.text($('#city').val());
	$selectValue.text($('#suburb').val());
	$selectValue.text($('#stock').val());
  $selectValue.text($('#stocksa').val());
  $selectValue.text($('#pay_method').val()); 
  $selectValue.text($('#province2').val());
	
    // Initialize Selectric and bind to 'change' event
		
	$('#province').selectric().on('change', function() {
      $selectValue.text($(this).val());
    });
	$('#city').selectric().on('change', function() {
      $selectValue.text($(this).val());
    });
	$('#suburb').selectric().on('change', function() {
      $selectValue.text($(this).val());
    });
	$('#stock').selectric().on('change', function() {
      $selectValue.text($(this).val());
    });
	$('#stocksa').selectric().on('change', function() {
      $selectValue.text($(this).val());
    });
  $('#pay_method').selectric().on('change', function() {
    $selectValue.text($(this).val());
  });
  $('#legal').selectric().on('change', function() {
    $selectValue.text($(this).val());
  });
  $('#province2').selectric().on('change', function() {
    $selectValue.text($(this).val());
  });
    /*------------------------------------*/
